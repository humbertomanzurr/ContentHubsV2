...
function Landing({onPath}){
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif"}}>
      {/* Nav */}
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 40px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{fontSize:15,fontWeight:700,color:C.text,letterSpacing:-0.3}}>ContentHub <span style={{color:C.accent,fontWeight:400,fontSize:13}}>Sidekick</span></div>
        <span onClick={()=>onPath("login")} style={{fontSize:12,color:C.accent,cursor:"pointer",fontWeight:600}}>Sign in</span>
      </div>

      {/* Hero */}
      <div style={{padding:"64px 40px 40px",textAlign:"center",maxWidth:720,margin:"0 auto"}}>
        {/* Not AI badge */}
        <div style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:11,padding:"4px 12px",borderRadius:20,background:"#FEE2E2",color:C.red,border:"1px solid #FECACA",marginBottom:20,fontWeight:600}}>
          🚫 Not an AI video generator
        </div>

        {/* Headline */}
        <div style={{fontSize:34,fontWeight:800,color:C.text,lineHeight:1.2,letterSpacing:-0.5,marginBottom:14}}>
          The planning tool for brands<br/>who <span style={{color:C.accent}}>actually</span> make their own videos.
        </div>

        {/* Subhead */}
        <div style={{fontSize:15,color:C.muted,lineHeight:1.65,maxWidth:500,margin:"0 auto 28px"}}>
          Organize ideas, script with AI, plan shoots, and track what's working — month after month. You do the filming. We make the planning smarter.
        </div>

        {/* 3 steps */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",maxWidth:560,margin:"0 auto 28px",border:`1px solid ${C.border}`,borderRadius:10,overflow:"hidden"}}>
          {[["01","Plan","Organize ideas into a pipeline"],["02","Produce","AI helps with scripts and shoot plans"],["03","Learn","Track metrics, improve every month"]].map(([n,t,d],i)=>(
            <div key={n} style={{padding:"14px 16px",borderRight:i<2?`1px solid ${C.border}`:"none",background:C.surface,textAlign:"left"}}>
              <div style={{fontSize:10,color:C.muted,marginBottom:3}}>{n}</div>
              <div style={{fontSize:12,fontWeight:700,color:C.text}}>{t}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2,lineHeight:1.4}}>{d}</div>
            </div>
          ))}
        </div>

        {/* How it works callout */}
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 16px",maxWidth:560,margin:"0 auto 44px",display:"flex",alignItems:"flex-start",gap:12,textAlign:"left"}}>
          <span style={{fontSize:20,flexShrink:0}}>💡</span>
          <div style={{fontSize:12,color:C.muted,lineHeight:1.6}}>
            <strong style={{color:C.text,fontWeight:600}}>How it works: </strong>
            You film and edit with your own camera and apps. ContentHub organizes your ideas, helps you write scripts and plan shoots with AI, and shows you which hooks and formats are actually driving views.
          </div>
        </div>
      </div>

      {/* Choose plan */}
      <div style={{textAlign:"center",marginBottom:20,fontSize:13,fontWeight:700,color:C.text}}>Choose your plan</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,maxWidth:660,margin:"0 auto",padding:"0 40px 60px"}}>
        {[
          {path:"creator",badge:"From $29/mo",badgeBg:"#EFF6FF",badgeColor:"#1D4ED8",icon:"🏢",iconBg:"#EFF6FF",title:"Business",desc:"For companies and in-house marketers making content for their own brand.",features:["One brand, one pipeline","AI script and shoot planning","Monthly progress dashboard","Hook and format analytics","Multi-platform tracking"],cta:"Start 14-day free trial",ctaBg:C.text,ctaColor:"#FFF",border:`1.5px solid ${C.accent}`},
          {path:"agency",badge:"Pro — from $79/mo",badgeBg:"#EDE9FE",badgeColor:"#7C3AED",icon:"🏛️",iconBg:"#EDE9FE",title:"Agency",desc:"For content teams managing multiple clients and brands.",features:["Unlimited clients","Team roles and permissions","Full production pipeline","Editor and CM accountability","Agency-wide analytics"],cta:"Start 14-day free trial",ctaBg:C.surface,ctaColor:C.text,border:`1px solid ${C.border}`},
        ].map(card=>(
          <div key={card.path} style={{background:C.surface,borderRadius:14,border:card.border,padding:24,display:"flex",flexDirection:"column",gap:0}}>
            <div style={{display:"inline-block",fontSize:10,fontWeight:600,padding:"3px 9px",borderRadius:20,background:card.badgeBg,color:card.badgeColor,marginBottom:14,width:"fit-content"}}>{card.badge}</div>
            <div style={{width:38,height:38,borderRadius:9,background:card.iconBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,marginBottom:12}}>{card.icon}</div>
            <div style={{fontSize:16,fontWeight:700,color:C.text,marginBottom:5}}>{card.title}</div>
            <div style={{fontSize:12,color:C.muted,lineHeight:1.5,marginBottom:14}}>{card.desc}</div>
            <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:18}}>
              {card.features.map(f=><div key={f} style={{display:"flex",alignItems:"flex-start",gap:6,fontSize:12,color:C.muted}}><span style={{color:C.green,flexShrink:0}}>✓</span>{f}</div>)}
            </div>
            <button onClick={()=>onPath(card.path)} style={{display:"block",width:"100%",padding:"10px 0",background:card.ctaBg,color:card.ctaColor,border:`1px solid ${card.ctaBg===C.surface?C.border:card.ctaBg}`,borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",marginTop:"auto"}}>
              {card.cta}
            </button>
            <div style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:10,cursor:"pointer"}}>▶ Watch a 90-second walkthrough</div>
          </div>
        ))}
      </div>
    </div>
...
function AgencyOnboarding({user,onComplete}){
  ...
  const finish=async()=>{
    ...
    if(clientName.trim()){
      await sbInsert("agency_clients",{id:"c"+uid(),workspace_id:wsId,name:clientName.trim(),emoji:clientEmoji,created_at:new Date().toISOString()});
    }
    setLoading(false);
    onComplete(wsId);
  };
  ...
  if(step===0)return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"min(500px,100%)",background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:shMd,padding:"40px 36px",textAlign:"center"}}>
        ...
        <div style={{fontSize:14,color:C.muted,lineHeight:1.7,marginBottom:20,maxWidth:380,margin:"0 auto 20px"}}>Manage content pipelines for all your clients in one place. Every client gets their own board, targets, and analytics.</div>
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:28,textAlign:"left",background:C.light,borderRadius:12,padding:"16px 18px"}}>
          {[["🗂️","One pipeline per client","Idea → Production → Editing → Published"],["📊","Progress at a glance","Monthly targets and health per client"],["✨","AI on every card","Script, shoot plan, and edit advice"]].map(([e,t,d])=>(
            <div key={t} style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:18}}>{e}</span>
              <div><div style={{fontSize:13,fontWeight:600,color:C.text}}>{t}</div><div style={{fontSize:11,color:C.muted}}>{d}</div></div>
            </div>
          ))}
        </div>
        <button onClick={()=>setStep(1)} style={{padding:"13px 32px",background:C.text,color:"#FFF",border:"none",borderRadius:10,fontSize:15,fontWeight:700,cursor:"pointer"}}>Set up my agency →</button>
      </div>
    </div>
  );
  ...
  if(step===1)return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"min(460px,100%)",background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:shMd,padding:"36px 32px"}}>
        ...
        <div style={{fontSize:11,fontWeight:600,color:C.accent,letterSpacing:0.5,marginBottom:8}}>1 of 2</div>
        <div style={{fontSize:20,fontWeight:800,color:C.text,marginBottom:6,letterSpacing:-0.3}}>What is your agency called?</div>
        <div style={{fontSize:13,color:C.muted,marginBottom:22,lineHeight:1.5}}>This becomes your workspace name. Your team will see it when they join.</div>
        <input autoFocus value={wsName} onChange={e=>setWsName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&wsName.trim()&&setStep(2)} placeholder="e.g. REVO Labs, Spark Media..." style={{...inp,fontSize:15,padding:"13px 14px",marginBottom:22}}/>
        <div style={{display:"flex",justifyContent:"flex-end"}}>
          <button onClick={()=>setStep(2)} disabled={!wsName.trim()} style={{padding:"10px 24px",background:wsName.trim()?C.text:C.border,color:wsName.trim()?"#FFF":C.muted,border:"none",borderRadius:8,fontSize:14,fontWeight:700,cursor:wsName.trim()?"pointer":"not-allowed"}}>Next →</button>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"min(460px,100%)",background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:shMd,padding:"36px 32px"}}>
        <div style={{display:"flex",gap:6,marginBottom:28}}>
          {[0,1].map(i=><div key={i} style={{height:4,borderRadius:20,flex:i===1?3:1,background:i===1?C.accent:"#BBF7D0",transition:"all .3s"}}/>)}
        </div>
        <div style={{fontSize:11,fontWeight:600,color:C.accent,letterSpacing:0.5,marginBottom:8}}>2 of 2</div>
        <div style={{fontSize:20,fontWeight:800,color:C.text,marginBottom:6,letterSpacing:-0.3}}>Add your first client</div>
        <div style={{fontSize:13,color:C.muted,marginBottom:22,lineHeight:1.5}}>You can add more later. Skip this if you prefer.</div>
        <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center",position:"relative"}}>
          <button onClick={()=>setShowEmoji(e=>!e)} style={{width:48,height:48,border:`1px solid ${C.border}`,borderRadius:10,background:C.light,fontSize:22,cursor:"pointer",flexShrink:0}}>{clientEmoji}</button>
          {showEmoji&&(
            <div style={{position:"absolute",top:"110%",left:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:8,zIndex:99,display:"flex",flexWrap:"wrap",width:196,gap:3,boxShadow:shMd}}>
              {EMOJIS.map(e=><button key={e} onClick={()=>{setClientEmoji(e);setShowEmoji(false);}} style={{width:32,height:32,border:"none",background:"none",cursor:"pointer",fontSize:18,borderRadius:6}}>{e}</button>)}
            </div>
          )}
          <input value={clientName} onChange={e=>setClientName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&finish()} placeholder="Client name e.g. Starbucks, Nike..." style={{...inp,flex:1,fontSize:14,padding:"12px 13px"}}/>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:22}}>
          <button onClick={finish} disabled={loading} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13,padding:0}}>Skip — go to dashboard</button>
          <button onClick={finish} disabled={loading||!clientName.trim()} style={{padding:"10px 24px",background:loading||!clientName.trim()?C.border:C.text,color:loading||!clientName.trim()?C.muted:"#FFF",border:"none",borderRadius:8,fontSize:14,fontWeight:700,cursor:loading||!clientName.trim()?"not-allowed":"pointer"}}>{loading?"Setting up...":"Let's go →"}</button>
        </div>
      </div>
    </div>
  );
...
// ── AGENCY STAGES ─────────────────────────────────────────────────────────────
const AGENCY_STAGES = [
  {id:"idea",       label:"Idea",              color:"#378ADD"},
  {id:"production", label:"Production",         color:"#8B5CF6"},
...
OUTPUT FORMAT:
Generate a concise editing advice checklist using ☐ checkboxes. Group by category. Be specific to this video's hook type, format, platform, and brand tone. Reference the script lines where helpful.
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui,sans-serif",padding:16}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 20px 60px rgba(0,0,0,.3)",width:"min(440px,100%)",padding:32,textAlign:"center"}}>
        ...
        <div style={{fontSize:17,fontWeight:800,color:C.text,marginBottom:8}}>✨ AI editing advice</div>
        <div style={{fontSize:13,color:C.muted,lineHeight:1.6,marginBottom:8}}>
          Based on your hook type, format, platform, and script — the AI generates a specific editing checklist for this video.
        </div>
                  ...
                  <div key={v.id} draggable
                    ...
                    <div style={{padding:"11px 12px",flex:1,minWidth:0}}>
                      ...
                      {(stage.id==="idea"||stage.id==="production")&&(
                        v.script
                          ?<button onClick={()=>setScriptCard(v)} style={{fontSize:10,padding:"4px 10px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:20,cursor:"pointer",color:C.green,fontWeight:600,fontSize:11,padding:"5px 12px"}}>📄 Script ready</button>
                          :<button onClick={()=>setAiCard(v)} style={{fontSize:10,padding:"4px 10px",background:"#EDE9FE",border:"1px solid #C4B5FD",borderRadius:20,cursor:"pointer",color:"#7C3AED",fontWeight:600,fontSize:11,padding:"5px 12px"}}>✨ Write script with AI</button>
                      )}
                      {stage.id==="production"&&(
                        v.shotList
...
function CreatorApp({user,profile,onLogout}){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      {tourStep===6&&<TourBubble step={6} total={6} emoji="📊" title="This is where it gets smart" body="After 7 days, add your metrics. The AI uses that data when writing your next script — by month 3 it knows your best hooks, formats, and posting days. The more you track, the smarter it gets." action="Check out Analytics →" onAction={()=>{finishTour();}} onSkip={finishTour} targetRef={refLock} side="left"/>}
      {showPublishPopup&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui"}}>
          <div style={{background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:"0 20px 60px rgba(0,0,0,.3)",width:"min(420px,95vw)",padding:32,textAlign:"center"}}>
            ...
            <div style={{fontSize:13,color:C.text,fontWeight:600,lineHeight:1.7,marginBottom:20,padding:"12px 16px",background:C.accent+"10",borderRadius:10,border:`1px solid ${C.accent}20`}}>
              The AI uses that data to write better scripts next time. The more you track, the smarter it gets. 📈
            </div>
...
function AgencyDashboard({clients,videos,targets,month,onMonthChange,onSelectClient,onAddClient,onSetTarget,businessProfile}){
  ...
  const addClient=async()=>{
    ...
    await onAddClient(newName.trim(),newEmoji);
  ...
  return(
    <div>
      ...
      <Card style={{marginBottom:16}}>
        ...
        {totalGoal>0&&<><div style={{background:C.light,borderRadius:20,height:9,overflow:"hidden",marginBottom:6}}><div style={{width:`${pct}%`,height:"100%",background:pct===100?C.green:C.accent,borderRadius:20,transition:"width .4s"}}/></div><div style={{fontSize:11,color:C.muted}}>{clients.length} clients · {totalGoal-totalPub} videos remaining</div></>}
      </Card>
  ...
  return(
    <div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        {goal>0&&<><div style={{background:C.light,borderRadius:20,height:8,overflow:"hidden",marginBottom:5}}><div style={{width:`${pct}%`,height:"100%",background:pct===100?C.green:C.accent,borderRadius:20,transition:"width .4s"}}/></div><div style={{fontSize:11,color:C.muted}}>{mVids.filter(v=>v.stage!=="published").length} in progress · {Math.max(0,goal-pub)} remaining</div></>}
      </Card>
...
function AgencyApp({user,profile,onLogout}){
  ...
  const addClient=useCallback(async(name,emoji)=>{
    const id="c"+uid();
    setClients(prev=>[...prev,{id,name,emoji,workspace_id:wsId}]);
