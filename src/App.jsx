  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shadowMd,width:"min(420px,95vw)",padding:26,fontFamily:"system-ui"}}>
        ...
        <div style={{fontSize:13,color:C.muted,marginBottom:16,lineHeight:1.5}}>Pega el link del video publicado (TikTok, Instagram, YouTube) para que el equipo pueda verlo directamente desde la tarjeta.</div>
        <div style={{marginBottom:18}}>
          <div style={{fontSize:11,color:C.muted,marginBottom:5,fontWeight:500}}>URL del video</div>
          <input autoFocus type="url" value={url} onChange={e=>setUrl(e.target.value)} onKeyDown={e=>e.key==="Enter"&&url.trim()&&onSave(card.id,url.trim())} placeholder="https://www.tiktok.com/@..." style={inp}/>
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn onClick={onClose}>Cancelar</Btn>
    ...
    <div draggable onDragStart={e=>{e.dataTransfer.setData("cardId",card.id);onDragStart&&onDragStart(card.id);}} onDragEnd={()=>onDragEnd&&onDragEnd()}
      ...
      <div style={{padding:"10px 12px",flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {/* Comments */}
          <button onClick={()=>onOpenComments&&onOpenComments(card)} style={{fontSize:10,padding:"3px 8px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.muted}}>💬</button>
          {/* Move month */}
  ...
  return(
    <div>
      ...
      {commentsCard&&<CommentsPanel
        card={commentsCard}
        currentUser={employees.find(e=>e.id===commentsCard?.clientId)?.name||"Admin"}
        currentRole={role}
