...
function SettingsPage({clients,employees,setClients,setEmployees,userRole}){
  ...
  const createUser=async()=>{
    ...
    if(nu.id){
      await sbInsert("profiles",{id:nu.id,name:newUser.name,role:newUser.role,client_id:newUser.clientId||null});
    }
...
// ── VIDEO HUB ─────────────────────────────────────────────────────────────────
function VideoHub({clients,videos,role,communityClientId}){
  ...
  if(role==="community"){
    ...
    return(
      <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif"}}>
        ...
        {/* Community tabs */}
        {selClientId&&<div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 24px",display:"flex",gap:0}}>
        ...
        <div style={{padding:24}}>
          {!selClientId
            ...
            :page==="videohub"
              ?<VideoHub clients={clients} videos={filtered} role="community" communityClientId={selClientId}/>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 24px",height:54,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,boxShadow:shadow}}>
          <div style={{fontSize:14,fontWeight:700,color:C.text}}>
            {page==="dashboard"?"📊 Dashboard":page==="pipeline"?"🗂 Pipeline":page==="videohub"?"🎬 Video Hub":page==="creative"?"🧠 Inteligencia Creativa":page==="team"?"👥 Equipo":page==="settings"?"⚙️ Configuración":activeClientObj?.name||""}
          </div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            {page!=="settings"&&page!=="pipeline"&&<DateRangePicker value={range} onChange={setRange}/>}
            {page!=="pipeline"&&page!=="settings"&&<Btn onClick={()=>setModal(activeClientObj?.id||"")} primary>+ Video</Btn>}
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          {page==="dashboard" &&<Dashboard  clients={clients}  videos={filtered} onClient={setPage}/>}
          {page==="videohub"  &&<VideoHub   clients={clients}  videos={filtered} role={role} communityClientId={null}/>}
