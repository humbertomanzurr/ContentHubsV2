...
function KanbanCard({card,clients,role,currentUser,onMove,onMetrics,onEarlyMetrics,onDelete,isDragging,onDragStart,onDragEnd,onMoveMonth,onAddVideoUrl,onOpenComments}){
  ...
  const isSentBack=card.stage==="edicion"&&card.revisionCount>0;
  ...
  return(
    <div draggable onDragStart={e=>{e.dataTransfer.setData("cardId",card.id);onDragStart&&onDragStart(card.id);}} onDragEnd={()=>onDragEnd&&onDragEnd()}
      ...
      <div style={{fontSize:11,color:C.muted,marginBottom:8,display:"flex",flexDirection:"column",gap:3}}>
        ...
        {(card.revisionCount||0)>0&&<div style={{color:"#F59E0B",fontWeight:700}}>🔄 Enviado a revisión {card.revisionCount}x</div>}
        {card.referenceUrl&&<a href={card.referenceUrl} target="_blank" rel="noreferrer" style={{color:C.accent,textDecoration:"none",fontWeight:600}}>📎 Ver referencia</a>}
        {card.videoUrl&&<a href={card.videoUrl} target="_blank" rel="noreferrer" style={{color:C.green,textDecoration:"none",fontWeight:600,background:"#DCFCE7",padding:"2px 8px",borderRadius:20,display:"inline-block"}}>▶ Ver video</a>}
      </div>
      {isSentBack&&<div style={{marginBottom:7,padding:"6px 9px",background:"#FEF3C7",borderRadius:7,border:"1px solid #FCD34D",fontSize:11,color:"#92400E",fontWeight:600}}>↩ Enviado a revisión — ver comentarios</div>}
      <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
        {nextStage&&!(nextStage.id==="publicado"&&role==="community")&&card.stage!=="metricas"&&!isAprobacion&&(
          <button onClick={()=>onMove(card.id,nextStage.id)} style={{fontSize:11,padding:"4px 10px",background:C.light,border:`1px solid ${C.border}`,borderRadius:6,cursor:"pointer",color:C.text}}>→ {nextStage.label}</button>
        )}
        {isAprobacion&&role==="admin"&&(
          <button onClick={()=>onMove(card.id,"metricas",true)} style={{fontSize:11,padding:"4px 10px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:6,cursor:"pointer",color:C.green,fontWeight:700}}>✓ Aprobar</button>
        )}
        {isAprobacion&&role==="admin"&&(
          <button onClick={()=>onOpenComments&&onOpenComments(card,"sendback")} style={{fontSize:11,padding:"4px 10px",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:6,cursor:"pointer",color:"#92400E",fontWeight:600}}>↩ Regresar</button>
        )}
        <button onClick={()=>onOpenComments&&onOpenComments(card)} style={{fontSize:11,padding:"4px 10px",background:C.light,border:`1px solid ${C.border}`,borderRadius:6,cursor:"pointer",color:C.muted}}>💬</button>
        {isMetricas&&unlocked&&<button onClick={()=>onMetrics(card)} style={{fontSize:11,padding:"4px 10px",background:C.accent,border:"none",borderRadius:6,cursor:"pointer",color:"#FFF",fontWeight:700}}>📊 Métricas</button>}
        {isMetricas&&!unlocked&&days!==null&&<div style={{fontSize:10,color:C.muted,padding:"4px 8px",background:C.light,borderRadius:6}}>🔒 {days}d restantes</div>}
        {isMetricas&&!unlocked&&days===null&&<button onClick={()=>onEarlyMetrics(card)} style={{fontSize:10,padding:"4px 8px",background:C.light,border:`1px solid ${C.border}`,borderRadius:6,cursor:"pointer",color:C.muted}}>¿Ya tienes datos?</button>}
        {role==="admin"&&<button onClick={()=>onMoveMonth&&onMoveMonth(card)} style={{fontSize:11,padding:"4px 9px",background:C.light,border:`1px solid ${C.border}`,borderRadius:6,cursor:"pointer",color:C.muted}}>📅</button>}
        {isAprobacion&&role==="admin"&&!card.videoUrl&&<button onClick={()=>onAddVideoUrl&&onAddVideoUrl(card)} style={{fontSize:11,padding:"4px 9px",background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:6,cursor:"pointer",color:C.accent,fontWeight:600}}>🔗</button>}
        {canDelete&&!confirmDel&&<button onClick={()=>setConfirmDel(true)} style={{fontSize:11,padding:"4px 9px",background:"#FEE2E2",border:"1px solid #FECACA",borderRadius:6,cursor:"pointer",color:C.red}}>🗑</button>}
        {canDelete&&confirmDel&&<div style={{display:"flex",gap:4,alignItems:"center"}}>
