...
const STAGES = [
  { id:"brief",      label:"Brief",           color:"#378ADD", communityCanMove:true  },
  { id:"produccion", label:"Production",       color:"#8B5CF6", communityCanMove:true  },
  { id:"edicion",    label:"Editing",          color:"#F59E0B", communityCanMove:true  },
  { id:"aprobacion", label:"Ready for Review", color:"#F97316", communityCanMove:true  },
  { id:"revision",   label:"Revision",         color:"#EC4899", communityCanMove:true  },
  { id:"publicado",  label:"Published",        color:"#059669", communityCanMove:false },
  { id:"metricas",   label:"Published",        color:"#059669", communityCanMove:false },
];
...
function AddCardModal({clients,employees,defaultClientId,role,activeMonth,onSave,onClose}){
  ...
  const[f,sf]=useState({clientId:defaultClientId||"",title:"",editor:"",dueDate:"",platform:"TikTok",month:activeMonth||curMonth(),referenceUrl:"",videoUrl:""});
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shadowMd,width:"min(480px,95vw)",padding:28,fontFamily:"system-ui,sans-serif"}}>
        ...
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,color:C.muted,marginBottom:4,fontWeight:500}}>📎 Referencia Canva (opcional)</div>
          <input type="url" value={f.referenceUrl} onChange={x=>set("referenceUrl",x.target.value)} placeholder="https://www.canva.com/design/..." style={inp}/>
          <div style={{fontSize:10,color:C.muted,marginTop:3}}>Link directo al slide de referencia para este video</div>
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:20}}>
          <Btn onClick={onClose}>Cancelar</Btn>
...
function MetricsModal({card,onSave,onClose}){
  ...
  const set=(k,v)=>sf(p=>({...p,[k]:v}));
  const[viewsErr,setViewsErr]=useState(false);
  const save=async()=>{
    if(!f.views||+f.views<=0){setViewsErr(true);return;}
    setViewsErr(false);
  ...
  const fld=(l,k,t="text",opts)=>(
    <div style={{marginBottom:12}}>
      <div style={{fontSize:11,color:k==="views"&&viewsErr?C.red:C.muted,marginBottom:4,fontWeight:500}}>{l}{k==="views"&&<span style={{color:C.red}}> *</span>}</div>
      {opts?<select value={f[k]} onChange={x=>set(k,x.target.value)} style={inp}><option value="">Seleccionar...</option>{opts.map(o=><option key={o}>{o}</option>)}</select>
           :<input type={t} value={f[k]} onChange={x=>{set(k,x.target.value);if(k==="views")setViewsErr(false);}} style={{...inp,border:k==="views"&&viewsErr?`1px solid ${C.red}`:inp.border}}/>}
      {k==="views"&&viewsErr&&<div style={{fontSize:10,color:C.red,marginTop:3}}>⚠️ Las vistas son requeridas antes de guardar métricas</div>}
    </div>
  );
...
const getComments = async (cardId) => {
  try {
    const r = await fetch(`${SB_URL}/rest/v1/card_comments?card_id=eq.${encodeURIComponent(cardId)}&order=created_at.asc&select=*`, {headers:_h()});
    return r.ok ? await r.json() : [];
  } catch(e) { return []; }
};
const addComment = async (cardId, authorName, authorRole, comment) => {
  try {
    await fetch(`${SB_URL}/rest/v1/card_comments`, {
      method:"POST",
      headers:_h({"Prefer":"return=minimal"}),
      body:JSON.stringify({card_id:cardId, author_name:authorName, author_role:authorRole, comment, created_at:new Date().toISOString()})
    });
  } catch(e) { console.error("addComment",e); }
};

// ── COMMENTS PANEL ────────────────────────────────────────────────────────────
function CommentsPanel({card, currentUser, currentRole, onClose, onSendBack}){
  ...
  useEffect(()=>{
    getComments(card.id).then(c=>{setComments(c||[]);setLoading(false);});
  },[card.id]);
  ...
  const send=async()=>{
    ...
    setSending(true);
    await addComment(card.id, currentUser, currentRole, input.trim());
    setComments(prev=>[...prev,{author_name:currentUser,author_role:currentRole,comment:input.trim(),created_at:new Date().toISOString()}]);
    setInput("");
    setSending(false);
  };
  ...
  const handleSendBack=async()=>{
    ...
    await addComment(card.id, currentUser, currentRole, "↩ Enviado a revisión: "+sendBackNote.trim());
    onSendBack&&onSendBack(card.id, sendBackNote.trim());
    onClose();
  };
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"stretch",justifyContent:"flex-end",zIndex:999,fontFamily:"system-ui"}}>
      <div style={{background:C.surface,width:"min(400px,100vw)",display:"flex",flexDirection:"column",boxShadow:"-4px 0 20px rgba(0,0,0,.15)"}}>
        {/* Header */}
        <div style={{padding:"16px 18px",borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            <div>
              <div style={{fontSize:14,fontWeight:800,color:C.text,marginBottom:2}}>💬 Comentarios</div>
              <div style={{fontSize:11,color:C.muted,lineHeight:1.3,maxWidth:280}}>{card.title}</div>
            </div>
            <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:22,cursor:"pointer",flexShrink:0}}>×</button>
          </div>
          {card.stage==="aprobacion"&&currentRole==="admin"&&!sendBackMode&&(
            <button onClick={()=>setSendBackMode(true)} style={{width:"100%",padding:"8px 0",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:700,color:"#92400E"}}>
              ↩ Regresar a edición
            </button>
          )}
          {sendBackMode&&(
            <div style={{marginTop:0}}>
              <div style={{fontSize:11,color:C.muted,marginBottom:6,fontWeight:600}}>¿Qué necesita corregirse? (obligatorio)</div>
              <textarea
                value={sendBackNote}
                onChange={e=>setSendBackNote(e.target.value)}
                placeholder="Ej: El hook necesita más energía, el corte en el segundo 4 es abrupto..."
                style={{...inp,height:76,resize:"none",fontSize:12,fontFamily:"system-ui",marginBottom:8}}
                autoFocus
              />
              <div style={{display:"flex",gap:7}}>
                <button onClick={()=>{setSendBackMode(false);setSendBackNote("");}} style={{flex:1,padding:"7px 0",background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,cursor:"pointer",fontSize:12,color:C.text}}>Cancelar</button>
                <button onClick={handleSendBack} disabled={!sendBackNote.trim()} style={{flex:1,padding:"7px 0",background:sendBackNote.trim()?"#F59E0B":"#E2E8F0",color:sendBackNote.trim()?"#FFF":C.muted,border:"none",borderRadius:7,cursor:sendBackNote.trim()?"pointer":"not-allowed",fontSize:12,fontWeight:700}}>Enviar a revisión ↩</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:10}}>
          {loading&&<div style={{textAlign:"center",color:C.muted,fontSize:12,marginTop:20}}>Cargando comentarios...</div>}
          {!loading&&comments.length===0&&<div style={{textAlign:"center",color:C.muted,fontSize:12,marginTop:40,lineHeight:1.7}}>Sin comentarios aún.<br/>Deja feedback para tu equipo aquí.</div>}
          {comments.map((c,i)=>{
            const isAdm=isAdmin(c.author_role);
            const isSendBack=c.comment?.startsWith("↩");
            return(
              <div key={i} style={{display:"flex",flexDirection:"column",alignItems:isAdm?"flex-start":"flex-end"}}>
                <div style={{fontSize:10,color:C.muted,marginBottom:3,textAlign:isAdm?"left":"right"}}>{c.author_name} · {timeAgo(c.created_at)}</div>
                <div style={{
                  maxWidth:"88%",
                  background:isSendBack?"#FEF3C7":isAdm?C.light:C.accent+"18",
                  border:`1px solid ${isSendBack?"#FCD34D":isAdm?C.border:C.accent+"30"}`,
                  borderRadius:isAdm?"10px 10px 10px 3px":"10px 10px 3px 10px",
                  padding:"9px 12px",
                  fontSize:12,
                  color:C.text,
                  lineHeight:1.6,
        ...
        <div style={{padding:"12px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,flexShrink:0}}>
          <textarea
            ...
            placeholder="Escribe un comentario... (Shift+Enter para nueva línea)"
            style={{...inp,resize:"none",height:38,fontSize:12,lineHeight:1.5,fontFamily:"system-ui",flex:1}}
          />
          <button onClick={send} disabled={sending||!input.trim()} style={{padding:"8px 14px",background:sending||!input.trim()?C.border:C.text,color:sending||!input.trim()?C.muted:"#FFF",border:"none",borderRadius:8,cursor:sending||!input.trim()?"not-allowed":"pointer",fontSize:12,fontWeight:700,flexShrink:0}}>
            Enviar
          </button>
        </div>
      </div>
...
// ── MOVE MONTH MODAL ──────────────────────────────────────────────────────────
function MoveMonthModal({card,onSave,onClose}){
  const months=[-2,-1,0,1,2].map(n=>{
    const d=new Date(NOW.getFullYear(),NOW.getMonth()+n,1);
    const val=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
    const label=d.toLocaleString("es-MX",{month:"long",year:"numeric"});
    return{val,label};
  });
  const[selected,setSelected]=useState(card.month||curMonth());
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shadowMd,width:"min(360px,95vw)",padding:26,fontFamily:"system-ui"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{fontSize:15,fontWeight:800,color:C.text}}>📅 Mover a otro mes</div>
          <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer"}}>×</button>
        </div>
        <div style={{fontSize:13,color:C.muted,marginBottom:16}}>"{card.title}"</div>
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
          {months.map(m=>(
            <button key={m.val} onClick={()=>setSelected(m.val)} style={{padding:"10px 14px",border:`1px solid ${selected===m.val?C.accent:C.border}`,borderRadius:8,background:selected===m.val?C.accent+"12":C.surface,color:selected===m.val?C.accent:C.text,cursor:"pointer",fontSize:13,fontWeight:selected===m.val?700:400,textAlign:"left"}}>
              {m.label} {m.val===card.month&&<span style={{fontSize:10,color:C.muted}}>(actual)</span>}
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn onClick={onClose}>Cancelar</Btn>
          <Btn onClick={()=>onSave(card.id,selected)} primary disabled={selected===card.month}>Mover ✓</Btn>
        </div>
      </div>
    </div>
  );
...
// ── ADD VIDEO URL MODAL ───────────────────────────────────────────────────────
function AddVideoUrlModal({card,onSave,onClose}){
  const[url,setUrl]=useState(card.videoUrl||"");
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shadowMd,width:"min(420px,95vw)",padding:26,fontFamily:"system-ui"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{fontSize:15,fontWeight:800,color:C.text}}>🔗 Agregar link del video</div>
          <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer"}}>×</button>
        </div>
        <div style={{fontSize:13,color:C.muted,marginBottom:16,lineHeight:1.5}}>Pega el link de Google Drive, Dropbox o cualquier plataforma donde esté el video listo para revisar.</div>
        <div style={{marginBottom:18}}>
          <div style={{fontSize:11,color:C.muted,marginBottom:5,fontWeight:500}}>URL del video</div>
          <input autoFocus type="url" value={url} onChange={e=>setUrl(e.target.value)} onKeyDown={e=>e.key==="Enter"&&url.trim()&&onSave(card.id,url.trim())} placeholder="https://drive.google.com/..." style={inp}/>
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn onClick={onClose}>Cancelar</Btn>
          <Btn onClick={()=>url.trim()&&onSave(card.id,url.trim())} primary>Guardar link ✓</Btn>
        </div>
      </div>
    </div>
  );
...
function KanbanCard({card,clients,role,currentUser,onMove,onMetrics,onEarlyMetrics,onDelete,isDragging,onDragStart,onDragEnd,onMoveMonth,onAddVideoUrl,onOpenComments}){
  ...
  const isPublished=isMetricas||isPublicado;
  const isAprobacion=card.stage==="aprobacion";
  const isRevision=card.stage==="revision";
  const isEdicion=card.stage==="edicion";
  const unlocked=metricsOK(card);
  const days=daysLeft(card);
  const isOverdue=card.dueDate&&new Date(card.dueDate)<NOW&&!isPublished;
  const stageInfo=STAGES.find(s=>s.id===card.stage);
  const canDelete=role==="admin"||(role==="community"&&card.stage==="brief");
  const isSentBack=isRevision&&(card.revisionCount||0)>0;
  ...
  return(
    <div draggable onDragStart={e=>{e.dataTransfer.setData("cardId",card.id);onDragStart&&onDragStart(card.id);}} onDragEnd={()=>onDragEnd&&onDragEnd()}
      style={{background:C.surface,borderRadius:10,marginBottom:8,border:`1px solid ${isAprobacion?"#FED7AA":isOverdue?"#FECACA":isPublished&&unlocked?"#BBF7D0":C.border}`,boxShadow:"0 1px 4px rgba(0,0,0,.06)",cursor:"grab",display:"flex",overflow:"hidden",opacity:isDragging?.5:1,transition:"box-shadow .15s,transform .15s"}}
      ...
      {/* Stage color stripe */}
      <div style={{width:4,background:stageInfo?.color||C.border,flexShrink:0,borderRadius:"10px 0 0 10px"}}/>
      <div style={{padding:"10px 12px",flex:1,minWidth:0}}>
        {/* Client tag */}
        <div style={{fontSize:10,fontWeight:600,color:stageInfo?.color,marginBottom:5,display:"flex",alignItems:"center",gap:5}}>
          <span style={{background:stageInfo?.color+"18",padding:"1px 7px",borderRadius:20}}>{client?.name||""}</span>
          {isAprobacion&&<span style={{background:"#FEF3C7",color:"#92400E",padding:"1px 7px",borderRadius:20}}>⏳ Pending review</span>}
          {isSentBack&&<span style={{background:"#FCE7F3",color:"#9D174D",padding:"1px 7px",borderRadius:20}}>↩ Sent back</span>}
        </div>
        {/* Title */}
        <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:6,lineHeight:1.3,wordBreak:"break-word"}}>{card.title}</div>
        {/* Meta row */}
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:7,fontSize:10,color:C.muted}}>
          {card.platform&&<span style={{background:C.light,padding:"2px 7px",borderRadius:20,border:`1px solid ${C.border}`}}>{card.platform}</span>}
          {card.editor&&<span>✂️ {card.editor}</span>}
          {card.dueDate&&<span style={{color:isOverdue?C.red:C.muted}}>📅 {card.dueDate}</span>}
        </div>
        {/* Reference + Video links */}
        {(card.referenceUrl||card.videoUrl)&&(
          <div style={{display:"flex",gap:8,marginBottom:7,flexWrap:"wrap"}}>
            {card.referenceUrl&&<a href={card.referenceUrl} target="_blank" rel="noreferrer" style={{fontSize:10,color:C.accent,textDecoration:"none",fontWeight:600,background:C.accent+"12",padding:"2px 8px",borderRadius:20}}>📎 Referencia</a>}
            {card.videoUrl&&<a href={card.videoUrl} target="_blank" rel="noreferrer" style={{fontSize:10,color:C.green,textDecoration:"none",fontWeight:600,background:"#DCFCE7",padding:"2px 8px",borderRadius:20}}>▶ Ver video</a>}
          </div>
        )}
        {/* Published metrics state */}
        {isPublished&&!unlocked&&days!==null&&(
          <div style={{marginBottom:7,padding:"5px 8px",background:"#F8FAFC",borderRadius:6,border:`1px solid ${C.border}`}}>
            <div style={{fontSize:10,color:C.muted,marginBottom:3}}>📅 Métricas disponibles en {days}d</div>
            <div style={{background:C.border,borderRadius:20,height:3,overflow:"hidden"}}><div style={{width:`${Math.min(100,Math.round(((7-(days||7))/7)*100))}%`,height:"100%",background:C.accent,borderRadius:20}}/></div>
          </div>
        )}
        {isPublished&&unlocked&&!isMetricas&&<div style={{marginBottom:7,fontSize:10,color:C.green,fontWeight:600}}>✅ Métricas agregadas</div>}
        {/* Actions */}
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          {/* Move forward — not for aprobacion (has its own approve btn) */}
          {nextStage&&!isAprobacion&&!isPublished&&!(nextStage.id==="publicado"&&role==="community")&&(
            <button onClick={()=>onMove(card.id,nextStage.id)} style={{fontSize:10,padding:"3px 9px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>→ {nextStage.label}</button>
          )}
          {/* Ready for Review actions */}
          {isAprobacion&&role==="admin"&&(
            <>
              <button onClick={()=>onMove(card.id,"publicado")} style={{fontSize:10,padding:"3px 9px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:20,cursor:"pointer",color:C.green,fontWeight:700}}>✓ Publicar</button>
              <button onClick={()=>onOpenComments&&onOpenComments(card,"sendback")} style={{fontSize:10,padding:"3px 9px",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:20,cursor:"pointer",color:"#92400E",fontWeight:600}}>Revisar →</button>
            </>
          )}
          {/* Revision — send back to Ready for Review */}
          {isRevision&&(
            <button onClick={()=>onMove(card.id,"aprobacion")} style={{fontSize:10,padding:"3px 9px",background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:20,cursor:"pointer",color:C.accent,fontWeight:600}}>→ Ready for Review</button>
          )}
          {/* Metrics */}
          {isPublished&&unlocked&&isMetricas&&<button onClick={()=>onMetrics(card)} style={{fontSize:10,padding:"3px 9px",background:C.accent,border:"none",borderRadius:20,cursor:"pointer",color:"#FFF",fontWeight:700}}>📊 Agregar métricas</button>}
          {isPublished&&!unlocked&&days===null&&<button onClick={()=>onEarlyMetrics(card)} style={{fontSize:10,padding:"3px 9px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.muted}}>¿Ya tienes datos?</button>}
          {/* Add video link in editing */}
          {isEdicion&&<button onClick={()=>onAddVideoUrl&&onAddVideoUrl(card)} style={{fontSize:10,padding:"3px 9px",background:card.videoUrl?"#DCFCE7":"#EFF6FF",border:`1px solid ${card.videoUrl?"#BBF7D0":"#BFDBFE"}`,borderRadius:20,cursor:"pointer",color:card.videoUrl?C.green:C.accent,fontWeight:600}}>{card.videoUrl?"▶ Video listo":"🔗 Agregar video"}</button>}
          {/* Comments — only on Ready for Review */}
          {isAprobacion&&<button onClick={()=>onOpenComments&&onOpenComments(card)} style={{fontSize:10,padding:"3px 8px",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:20,cursor:"pointer",color:"#92400E",fontWeight:600}}>💬 Comentarios</button>}
          {/* Move month */}
          {role==="admin"&&<button onClick={()=>onMoveMonth&&onMoveMonth(card)} style={{fontSize:10,padding:"3px 8px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.muted}}>📅</button>}
          {/* Delete */}
          {canDelete&&!confirmDel&&<button onClick={()=>setConfirmDel(true)} style={{fontSize:10,padding:"3px 8px",background:"#FEE2E2",border:"1px solid #FECACA",borderRadius:20,cursor:"pointer",color:C.red}}>🗑</button>}
          {canDelete&&confirmDel&&<div style={{display:"flex",gap:3}}>
            <button onClick={()=>{onDelete(card.id);setConfirmDel(false);}} style={{fontSize:10,padding:"3px 8px",background:C.red,border:"none",borderRadius:20,cursor:"pointer",color:"#FFF",fontWeight:700}}>Sí</button>
            <button onClick={()=>setConfirmDel(false)} style={{fontSize:10,padding:"3px 8px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>No</button>
          </div>}
        </div>
      </div>
    </div>
...
function PipelineOverview({clients,cards,videos,targets,role,onSelect,onSetTargets,activeMonth,onMonthChange}){
  ...
  const activeClients=clients.filter(c=>c.status!=="archived");
  const publishedThisMonth=id=>{
    const fromVideos=videos.filter(v=>v.clientId===id&&v.publishDate?.startsWith(month)).length;
    const fromMetricas=cards.filter(c=>c.clientId===id&&c.month===month&&(c.stage==="metricas"||c.stage==="publicado")).length;
    return fromVideos+fromMetricas;
  };
...
// ── PIPELINE BOARD ────────────────────────────────────────────────────────────
function PipelineBoard({clientId,clients,employees,cards,videos,targets,role,currentUser,onAddCard,onMoveCard,onMetrics,onDeleteCard,onBack,activeMonth,onMonthChange,onSetTargets}){
  const month=activeMonth||curMonth();
  ...
  const[showBoardTarget,setShowBoardTarget]=useState(false);
  const[boardTargetVal,setBoardTargetVal]=useState(target||0);
  const[moveMonthCard,setMoveMonthCard]=useState(null);
  const[addVideoUrlCard,setAddVideoUrlCard]=useState(null);
  const[commentsCard,setCommentsCard]=useState(null);
  const[commentsSendBack,setCommentsSendBack]=useState(false);
  ...
  const published=videos.filter(v=>v.clientId===clientId&&v.publishDate?.startsWith(month)).length
    + cards.filter(c=>c.clientId===clientId&&c.month===month&&(c.stage==="metricas"||c.stage==="publicado")).length;
  const pct=target>0?Math.min(100,Math.round(published/target*100)):null;
  const behind=target>0&&published<Math.floor(target*0.5);
  const cardsInStage=sid=>{
    if(sid==="publicado") return monthCards.filter(c=>c.stage==="publicado"||c.stage==="metricas");
    return monthCards.filter(c=>c.stage===sid);
  };
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div>
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              ...
              {selClient?.canva_url&&<a href={selClient.canva_url} target="_blank" rel="noreferrer" style={{fontSize:11,color:C.accent,background:C.accent+"12",padding:"3px 10px",borderRadius:20,textDecoration:"none",fontWeight:600,border:`1px solid ${C.accent}22`}}>🎨 Deck de ideas</a>}
            </div>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <MonthNav value={month} onChange={onMonthChange}/>
          {role==="admin"&&<Btn onClick={()=>setShowBoardTarget(true)}>🎯 {target>0?`Meta: ${target}`:"Definir meta"}</Btn>}
          <Btn primary onClick={()=>setShowAddCard(true)}>+ Nuevo video</Btn>
        </div>
      </div>
      {target===0&&role==="admin"&&(
        <div style={{background:"#FFFBEB",border:`1px solid ${C.amber}40`,borderRadius:10,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:13,color:C.muted}}>🎯 No hay meta definida para este cliente este mes.</span>
          <button onClick={()=>setShowBoardTarget(true)} style={{padding:"5px 12px",background:C.amber,color:"#FFF",border:"none",borderRadius:7,fontSize:12,fontWeight:700,cursor:"pointer"}}>Definir meta</button>
        </div>
      )}
      {target>0&&(
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,overflowX:"auto",paddingBottom:4}}>
        {STAGES.filter(s=>s.id!=="metricas").map(s=>{const n=cardsInStage(s.id).length;return(
      ...
      <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:12}}>
        {STAGES.filter(s=>s.id!=="metricas").map(stage=>(
          <div key={stage.id}
            onDragOver={e=>e.preventDefault()} onDrop={e=>handleDrop(e,stage.id)}
            onDragEnter={e=>{e.currentTarget.style.background=stage.id==="aprobacion"?"#FEF3C7":"#EFF6FF";}}
            ...
            {cardsInStage(stage.id).map(card=>(
              <KanbanCard key={card.id} card={card} clients={clients} role={role}
                ...
                onMoveMonth={c=>setMoveMonthCard(c)} onAddVideoUrl={c=>setAddVideoUrlCard(c)} onOpenComments={(c,mode)=>{setCommentsCard(c);setCommentsSendBack(mode==="sendback");}}/>
            ))}
      ...
      {commentsCard&&<CommentsPanel
        card={commentsCard}
        currentUser={currentUser||"Admin"}
        currentRole={role}
        onClose={()=>{setCommentsCard(null);setCommentsSendBack(false);}}
        onSendBack={async(id,note)=>{
          await sbUpdate("cards","id",id,{stage:"edicion",revision_count:(cards.find(c=>c.id===id)?.revisionCount||0)+1});
          onMoveCard&&onMoveCard(id,"edicion");
          setCommentsCard(null);setCommentsSendBack(false);
        }}
      />}
      {moveMonthCard&&<MoveMonthModal card={moveMonthCard} onSave={async(id,newMonth)=>{await sbUpdate("cards","id",id,{month:newMonth});setMoveMonthCard(null);}} onClose={()=>setMoveMonthCard(null)}/>}
      {addVideoUrlCard&&<AddVideoUrlModal card={addVideoUrlCard} onSave={async(id,url)=>{await sbUpdate("cards","id",id,{video_url:url});setAddVideoUrlCard(null);}} onClose={()=>setAddVideoUrlCard(null)}/>}
      {showBoardTarget&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}}>
          <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shadowMd,width:"min(360px,95vw)",padding:26,fontFamily:"system-ui"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{fontSize:15,fontWeight:800,color:C.text}}>🎯 Meta de {selClient?.name}</div>
              <button onClick={()=>setShowBoardTarget(false)} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer"}}>×</button>
            </div>
            <div style={{fontSize:13,color:C.muted,marginBottom:16}}>¿Cuántos videos debe publicar este cliente en {month}?</div>
            <input type="number" min="0" max="60" value={boardTargetVal} onChange={e=>setBoardTargetVal(+e.target.value)} style={{...inp,textAlign:"center",fontSize:22,fontWeight:700,padding:"12px",marginBottom:16}}/>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn onClick={()=>setShowBoardTarget(false)}>Cancelar</Btn>
              <Btn onClick={async()=>{await sbUpsert("targets",[{month,client_id:clientId,target:boardTargetVal}],"month,client_id");onSetTargets&&onSetTargets();setShowBoardTarget(false);}} primary>Guardar meta ✓</Btn>
            </div>
          </div>
        </div>
      )}
...
// ── PIPELINE PAGE ─────────────────────────────────────────────────────────────
function PipelinePage({clients,employees,cards,videos,targets,role,currentUser,onSetTargets,onMoveCard,onDeleteCard,communityClientId,initialClient,onClearInitialClient,activeMonth,onMonthChange}){
  ...
  if(!selectedClient)return <PipelineOverview clients={clients} cards={cards} videos={videos} targets={targets} role={role} onSelect={setSelectedClient} onSetTargets={onSetTargets} activeMonth={activeMonth} onMonthChange={onMonthChange}/>;
  return <PipelineBoard clientId={selectedClient} clients={clients} employees={employees} cards={cards} videos={videos} targets={targets} role={role} currentUser={currentUser} onAddCard={()=>{}} onMoveCard={onMoveCard} onMetrics={()=>{}} onDeleteCard={onDeleteCard} onBack={communityClientId?null:()=>setSelectedClient(null)} activeMonth={activeMonth} onMonthChange={onMonthChange} onSetTargets={onSetTargets}/>;
}
...
function SettingsPage({clients,employees,setClients,setEmployees,userRole}){
  ...
  const blankC={name:"",industry:"Retail",status:"active",am:"",goal:"",emoji:"",canva_url:""};
  ...
  const startEditC=c=>{setCf({name:c.name,industry:c.industry,status:c.status,am:c.am,goal:c.goal,canva_url:c.canva_url||""});setEditItem({type:"client",data:c});setShowAdd(true);};
          ...
          {showAdd&&(
            <Card style={{marginBottom:20,borderLeft:`4px solid ${C.accent}`}}>
              ...
              <div style={{marginTop:12}}>
                <div style={{fontSize:11,color:C.muted,marginBottom:4}}>🎨 Canva Deck URL (referencias del cliente)</div>
                <input type="url" value={cf.canva_url||""} onChange={x=>setC("canva_url",x.target.value)} placeholder="https://www.canva.com/design/..." style={inp}/>
              </div>
...
function MonthlyDashboard({clients,cards,videos,targets,onGoToPipeline,activeMonth,onMonthChange}){
  ...
  const publishedCount = clientId =>
    ...
    + cards.filter(c=>c.clientId===clientId&&c.month===month&&(c.stage==="publicado"||c.stage==="metricas")).length;
              ...
              <div key={v.id} style={{background:C.surface,borderRadius:12,border:`1px solid ${C.border}`,boxShadow:shadow,overflow:"hidden"}}>
                <div style={{padding:"14px 16px 12px",borderBottom:`1px solid ${C.border}`}}>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:4}}>
                    ...
                    {v.views===0&&<span style={{fontSize:9,fontWeight:700,background:"#FEF3C7",color:"#92400E",padding:"2px 7px",borderRadius:20,border:"1px solid #FCD34D"}}>⚠️ Vistas en 0</span>}
                  </div>
                </div>
      ...
      <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          {page==="videohub"
            ...
            :<PipelinePage clients={clients} employees={employees} cards={cards} videos={videos} targets={targets} role="community" currentUser={user?.email||"Community"} communityClientId={null} onSetTargets={()=>{}} onMoveCard={moveCard} onDeleteCard={deleteCard} activeMonth={activeMonth} onMonthChange={setActiveMonth}/>
          }
        </div>
      </div>
    );
    ...
    <div style={{display:"flex",height:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",color:C.text,overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          ...
          {page==="pipeline"  &&<PipelinePage clients={clients} employees={employees} cards={cards} videos={videos} targets={targets} role="admin" currentUser={profile?.name||user?.email||"Admin"} communityClientId={null} onSetTargets={onSetTargets} onMoveCard={moveCard} onDeleteCard={deleteCard} initialClient={pipelineClient} onClearInitialClient={()=>setPipelineClient(null)} activeMonth={activeMonth} onMonthChange={setActiveMonth}/>}
