...
const sbSignIn = async (email, password) => {
  try {
    ...
    localStorage.setItem("sb_auth", JSON.stringify({token:d.access_token, user:d.user}));
...
// ── PIPELINE PAGE ─────────────────────────────────────────────────────────────
function PipelinePage({clients,employees,cards,videos,targets,role,onSetTargets,onMoveCard,onDeleteCard,communityClientId,initialClient,onClearInitialClient}){
...
function MonthlyDashboard({clients,cards,videos,targets,onGoToPipeline}){
  ...
  const healthColor    = healthPct===null?C.muted:healthPct>=80?C.green:healthPct>=50?C.amber:C.red;
  const healthLabel    = healthPct===null?"Sin metas definidas":healthPct===100?"✅ Meta cumplida":healthPct>=50?"🟡 En camino":"🔴 Por debajo";
    ...
    <div>
      ...
      <div style={{background:C.surface,borderRadius:14,padding:24,border:`1px solid ${C.border}`,boxShadow:shadow,marginBottom:24}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:12}}>
          <div>
            <div style={{fontSize:12,color:C.muted,letterSpacing:1,fontWeight:600,marginBottom:4}}>SALUD DE LA AGENCIA</div>
            <div style={{fontSize:28,fontWeight:900,color:healthColor}}>{healthLabel}</div>
          </div>
          <div style={{textAlign:"right"}}>
        ...
        <div style={{background:C.light,borderRadius:20,height:12,overflow:"hidden",marginBottom:10}}>
          <div style={{width:`${healthPct||0}%`,height:"100%",background:healthColor,borderRadius:20,transition:"width .5s"}}/>
        </div>
        <div style={{display:"flex",gap:20,fontSize:12,color:C.muted}}>
          <span>✅ {totalPublished} publicados</span>
          <span>🎬 {totalPipeline} en pipeline</span>
          {totalTarget>0&&<span>🎯 {Math.max(0,totalTarget-totalPublished)} restantes</span>}
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
        {activeClients.map(c=>{
          ...
          const behind=tgt>0&&pub<Math.floor(tgt*0.5);
          const color=behind?C.red:pct===100?C.green:C.amber;
...
function AnalyticsPage({videos,clients}){
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24}}>
        <Kpi emoji="👁" v={fmt(videos.reduce((s,v)=>s+v.views,0))} l="Vistas totales"/>
        <Kpi emoji="🎬" v={videos.length} l="Videos publicados"/>
        <Kpi emoji="💹" v={videos.length?(videos.reduce((s,v)=>s+(v.views>0?(v.likes+v.comments+v.shares+v.saves)/v.views:0),0)/videos.length*100).toFixed(1)+"%":"—"} l="Engagement promedio" color={C.accent}/>
        <Kpi emoji="📡" v={(()=>{const w=videos.filter(v=>v.paraTi!=null);return w.length?Math.round(w.reduce((s,v)=>s+v.paraTi,0)/w.length)+"%":"—";})())} l="% Para Ti promedio" color={C.green}/>
      </div>
...
function VideoHub({clients,videos,role,communityClientId,onDeleteVideo}){
  const[selectedClient,setSelectedClient]=useState(communityClientId||"all");
  ...
  const filtered=videos
    .filter(v=>(selectedClient==="all"||v.clientId===selectedClient))
    .filter(v=>[v.title,v.creator,v.hook,v.format].some(x=>x?.toLowerCase().includes(q.toLowerCase())))
    .sort((a,b)=>sortBy==="views"?b.views-a.views:new Date(b.publishDate)-new Date(a.publishDate));
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}>
        <div>
          ...
          <div style={{fontSize:13,color:C.muted,marginTop:4}}>{filtered.length} videos publicados</div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
        {role==="admin"&&(
          <select value={selectedClient} onChange={e=>setSelectedClient(e.target.value)}
            style={{padding:"8px 12px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,fontSize:13,color:C.text,cursor:"pointer",boxShadow:shadow}}>
            <option value="all">Todos los clientes</option>
            {activeClients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        )}
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="🔍 Buscar videos..."
          style={{...inp,width:220,fontSize:13,padding:"8px 12px"}}/>
      ...
      {filtered.length===0
        ?<Card style={{textAlign:"center",padding:48}}>
          ...
          <div style={{fontSize:15,fontWeight:700,color:C.text}}>Sin videos</div>
          <div style={{fontSize:13,color:C.muted,marginTop:6}}>Los videos aparecen aquí después de agregar métricas en el pipeline</div>
         </Card>
              ...
              <div key={v.id} style={{background:C.surface,borderRadius:12,border:`1px solid ${C.border}`,boxShadow:shadow,overflow:"hidden"}}>
                ...
                <div style={{padding:"14px 16px 12px",borderBottom:`1px solid ${C.border}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    <span style={{fontSize:10,fontWeight:700,background:C.accent+"18",color:C.accent,padding:"2px 8px",borderRadius:20}}>{clName(v.clientId)}</span>
                ...
                <div style={{padding:"10px 16px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{fontSize:11,color:C.muted}}>{v.creator&&`🎭 ${v.creator}`}</div>
                    {role==="admin"&&(
                      confirmDel===v.id
                  ...
                  {v.url
                    ?<a href={v.url} target="_blank" rel="noreferrer"
                        style={{fontSize:12,fontWeight:700,color:C.accent,textDecoration:"none",background:C.accent+"12",padding:"5px 12px",borderRadius:20,border:`1px solid ${C.accent}30`}}>
                        Ver en TikTok →
                       </a>
      ...
      <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          {page==="videohub"
            ...
            :<PipelinePage clients={clients} employees={employees} cards={cards} videos={videos} targets={targets} role="community" communityClientId={null} onSetTargets={()=>{}} onMoveCard={moveCard} onDeleteCard={deleteCard}/>
          }
        </div>
      </div>
    );
    ...
    <div style={{display:"flex",height:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",color:C.text,overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          ...
          {page==="pipeline"  &&<PipelinePage clients={clients} employees={employees} cards={cards} videos={videos} targets={targets} role="admin" communityClientId={null} onSetTargets={onSetTargets} onMoveCard={moveCard} onDeleteCard={deleteCard} initialClient={pipelineClient} onClearInitialClient={()=>setPipelineClient(null)}/>}
