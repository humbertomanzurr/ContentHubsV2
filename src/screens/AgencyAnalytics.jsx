    ...
    <div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}}>
          ...
          <button onClick={()=>printClientReport(client,scoped,measured,month)} style={{padding:"5px 11px",border:`0.5px solid ${C.border}`,borderRadius:20,background:C.surface,cursor:"pointer",fontSize:11,color:C.muted}}>{t("Report")}</button>
          <button onClick={onOpenPipeline} style={{padding:"5px 11px",border:`0.5px solid ${C.border}`,borderRadius:20,background:C.surface,cursor:"pointer",fontSize:11,color:C.muted}}>{t("Pipeline \u2192")}</button>
        </div>
      </div>

      {measured.length===0?(
        <Card><div style={{textAlign:"center",padding:"44px 20px"}}>
          <div style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:5}}>{t("No metrics recorded yet")}</div>
        ...
        <>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            <div>
              <div style={{fontSize:9,fontWeight:600,color:BRAND.green,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{t("Best performer")}</div>
              {vidCard(best,BRAND.green)}
            </div>
            <div>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{t("Weakest performer")}</div>
    ...
    <div>
      <Card pad={0} style={{marginBottom:16,overflow:"hidden"}}>
        ...
        <div style={{padding:"16px 20px 18px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:14,marginBottom:16,flexWrap:"wrap"}}>
            <div>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>{t("Agency health")}</div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:7}}>
              ...
              <button onClick={()=>printAgencyReport(clients,videos,targets,month)}
                style={{padding:"5px 11px",border:`0.5px solid ${C.border}`,borderRadius:7,background:C.surface,cursor:"pointer",fontSize:11,color:C.muted}}>{t("Agency report")}</button>
            </div>
          </div>

          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>{t("Delivery")}</div>
          ...
          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>{t("Reach")}</div>
                ...
                {stuck.slice(0,5).map(v=>{
                  ...
                  return(
                    <div key={v.id} style={{display:"flex",alignItems:"center",gap:9,background:"#FFF7ED",border:"0.5px solid #FED7AA",borderRadius:7,padding:"7px 10px"}}>
                      ...
                      <span style={{fontSize:10,color:C.muted,flexShrink:0}}>{c?c.name:""} · {t((AGENCY_STAGES.find(s=>s.id===v.stage)||{}).label||"")}</span>
                    </div>
                  );
                })}
      ...
      {(board.length>0||chBoard.length>0)&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12,marginBottom:16}}>
          {board.length>0&&(
            <Card pad={16}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>{t("Client momentum")}</div>
          ...
          {chBoard.length>0&&(
            <Card pad={16}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>{t("Channels")}</div>
      ...
      <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:9}}>{t("By client")}</div>
      {clients.length===0&&<Card><div style={{textAlign:"center",padding:"34px 20px",color:C.muted,fontSize:13}}>{t("Add a client to start seeing analytics.")}</div></Card>}
            ...
            <Card key={c.id} pad={16}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:13}}>
                ...
                <button onClick={()=>printClientReport(c,cv,cm,month)}
                  style={{fontSize:10,padding:"5px 10px",border:`0.5px solid ${C.border}`,borderRadius:20,background:C.surface,cursor:"pointer",color:C.muted,whiteSpace:"nowrap",flexShrink:0}}>{t("Report")}</button>
              </div>
              ...
              <div style={{display:"flex",gap:7,marginBottom:12}}>
                <div style={{flex:1,background:C.light,borderRadius:7,padding:"8px 9px",borderLeft:`3px solid ${BRAND.green}`}}>
                  <div style={{fontSize:9,color:C.muted}}>{t("Published")}</div>
                ...
                <div style={{flex:1,background:C.light,borderRadius:7,padding:"8px 9px",borderLeft:`3px solid ${cRw.pct===null?C.border:cRw.pct<=20?BRAND.green:cRw.pct<=40?BRAND.yellow:BRAND.red}`}}>
                  <div style={{fontSize:9,color:C.muted}}>{t("Rework")}</div>
...
const printAgencyReport=(clients,videos,targets,month)=>{
  ...
  const stuckRows=stuck.slice(0,10).map(v=>{
    const c=clients.find(x=>x.id===v.clientId);
    return `<tr><td>${esc(v.title)}</td><td>${esc(c?c.name:"")}</td><td>${esc(t((AGENCY_STAGES.find(s=>s.id===v.stage)||{}).label||"")||v.stage)}</td><td>${v.daysInStage} days</td></tr>`;
  }).join("");
  ...
  w.document.write(`<html><head><title>Agency report — ${esc(monthLabel(month))}</title><style>
    ...
    <h1>{t("Agency report")}</h1>
    <div class="meta">${esc(monthLabel(month))} · ${clients.length} client${clients.length===1?"":"s"}</div>
    <h2>{t("Delivery")}</h2>
    <div class="grid">
      <div class="kpi"><div class="l">{t("Published")}</div><div class="v">${pub.length}</div></div>
      <div class="kpi"><div class="l">{t("On time")}</div><div class="v">${ot.pct===null?"—":ot.pct+"%"}</div><div class="s">${ot.total?ot.n+" of "+ot.total:"no target dates"}</div></div>
      <div class="kpi"><div class="l">{t("Brief \u2192 published")}</div><div class="v">${ct.days===null?"—":ct.days+"d"}</div><div class="s">${ct.n?"typical across "+ct.n:""}</div></div>
      <div class="kpi"><div class="l">{t("Rework")}</div><div class="v">${rw.pct===null?"—":rw.pct+"%"}</div><div class="s">${rw.total?rw.n+" of "+rw.total+" sent back":""}</div></div>
    </div>
    <h2>{t("Reach")}</h2>
    <div class="grid">
      <div class="kpi"><div class="l">{t("Total views")}</div><div class="v">${meas.length?fmt(meas.reduce((a,v)=>a+v.views,0)):"—"}</div></div>
      <div class="kpi"><div class="l">{t("Typical video")}</div><div class="v">${meas.length?fmt(median(meas.map(v=>v.views))):"—"}</div><div class="s">{t("half do better, half worse")}</div></div>
    </div>
    ${rows?`<h2>{t("By client")}</h2><table><tr><th>Client</th><th>{t("Published")}</th><th>Typical</th><th>Change</th><th>{t("Rework")}</th></tr>${rows}</table>`:""}
...
const printClientReport=(client,allVids,measured,month)=>{
  ...
  w.document.write(`<html><head><title>${esc(client.name)} — ${esc(monthLabel(month))}</title><style>
    ...
    <h2>Summary</h2>
    <div class="grid">
      <div class="kpi"><div class="l">{t("Published")}</div><div class="v">${pub}</div></div>
      <div class="kpi"><div class="l">{t("Total views")}</div><div class="v">${views>0?fmt(views):"—"}</div></div>
    ...
    <div class="grid">
      <div class="kpi"><div class="l">{t("Top hook")}</div><div class="v">${hook?esc(hook.value):"—"}</div><div class="n">${hook?`across ${hook.n} video${hook.n===1?"":"s"}`:"no data yet"}</div></div>
      <div class="kpi"><div class="l">{t("Top format")}</div><div class="v">${format?esc(format.value):"—"}</div><div class="n">${format?`across ${format.n} video${format.n===1?"":"s"}`:"no data yet"}</div></div>
    </div>
    ${chRows?`<h2>{t("By channel")}</h2><table><tr><th>Channel</th><th>Videos</th><th>Views</th><th>Best hook</th></tr>${chRows}</table>`:""}
    ${top?`<h2>Top videos</h2><table><tr><th>Title</th><th>Channel</th><th>{t("Hook")}</th><th>Views</th></tr>${top}</table>`:""}
