...
const STATUSES=[
  ...
  {id:"contactado", label:"Contactado", color:BRAND.blue},
  {id:"respondio",  label:"Respondió",  color:"#0EA5E9"},
  {id:"negociando", label:"Negociando", color:BRAND.yellow},
  {id:"contratado", label:"Contratado", color:BRAND.green},
  {id:"descartado", label:"Descartado", color:C.red},
];
...
const SEARCH_HINTS=[
  {key:"niche",   label:"nicho",      chips:["Fitness","Belleza","Comida","Moda","Bienestar","Papás y mamás","Tecnología","Viajes"]},
  {key:"city",    label:"ciudad",     chips:["CDMX","Guadalajara","Monterrey","Puebla","Querétaro","Todo México"]},
  {key:"size",    label:"tamaño",     chips:["Cuentas chicas","5k a 50k","50k a 200k","Más de 200k"]},
  {key:"platform",label:"plataforma", chips:["TikTok","Instagram","YouTube"]},
];
...
function CreatorHub({workspaceId,clients}){
  ...
  const composed=(()=>{
    ...
    return bits.length?`Creadores de ${bits.join(", ")}`:"";
  })();
  ...
  const addLink=async()=>{
    ...
    if(!p){setLinkErr(t("That doesn't look like a profile link."));return;}
    setLinkErr(null);
  ...
  const runSearch=async()=>{
    ...
    try{
      const sys=`Encuentras creadores de contenido reales en redes sociales.

Búsqueda: ${q}

TU PRIMERA ACCIÓN ES BUSCAR. No respondas de memoria. Usa la herramienta de búsqueda web antes de escribir nada.

REGLAS:
- "profile_url" DEBE ser un link real que apareció en tus resultados de búsqueda. Nunca lo inventes ni lo construyas por patrón.
- Si no encuentras el perfil real, deja fuera a ese creador. Menos resultados reales es lo correcto.
- No inventes números de seguidores ni de engagement. No los incluyas.

Devuelve SOLO un arreglo JSON, sin markdown:
...
Hasta 12 resultados.`;
      ...
      setResults(verified);
      if(!verified.length)setSearchErr(t("Nothing came back with a real profile link."));
  ...
  const addSelectedToList=async(listId)=>{
    ...
    if(rows.length)await sbUpsert("creator_list_members",rows,"list_id,creator_id");
    ...
    return(
      <div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,flexWrap:"wrap"}}>
          <button onClick={()=>{setOpenList(null);load();}} style={{padding:"6px 12px",border:`1px solid ${C.border}`,borderRadius:8,background:C.surface,cursor:"pointer",fontSize:12,color:C.muted}}>← {t("Lists")}</button>
          ...
          <span style={{marginLeft:"auto",fontSize:11,color:C.muted}}>{rows.length} {t("creators")}</span>
        </div>
        ...
        <Card pad={14} style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:12,flexWrap:"wrap",fontSize:10,color:C.muted}}>
            {counts.filter(c=>c.n>0).map(c=><span key={c.id} style={{color:c.id==="contratado"?BRAND.green:C.muted}}>{c.n} {t(c.label)}</span>)}
            {rows.length===0&&<span>{t("Nobody here yet.")}</span>}
          </div>
        </Card>
            ...
            <Card key={r.creator_id} pad={13}>
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                ...
                {r.creator.followers>0&&(
                  <span style={{fontSize:10,color:C.muted}}>{r.creator.followers.toLocaleString("es-MX")} {t("followers")}
                    {r.creator.checked_at&&<span style={{color:C.muted}}> · {t("checked")} {r.creator.checked_at}</span>}
                  </span>
                )}
                ...
                <button onClick={()=>setDataFor(r.creator)} style={{fontSize:10,padding:"4px 9px",borderRadius:20,border:`0.5px solid ${C.border}`,background:C.surface,cursor:"pointer",color:C.muted}}>{t("+ data")}</button>
    ...
    <div>
      <Card pad={0} style={{marginBottom:16,overflow:"hidden"}}>
        ...
        <div style={{padding:"16px 20px 18px"}}>
          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>{t("Prospecting")}</div>
          <div style={{fontSize:17,fontWeight:600,color:C.text,letterSpacing:-0.2,marginBottom:4}}>{t("Who are you looking for?")}</div>
          <div style={{fontSize:12,color:C.muted,lineHeight:1.55,marginBottom:14}}>{t("Describe the creator you need, or paste the link of one you already found.")}</div>

          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{t("Build your search")}</div>
          {SEARCH_HINTS.map(h=>(
          ...
          <div style={{position:"relative",marginTop:12,marginBottom:10}}>
            <textarea value={query||composed} onChange={e=>setQuery(e.target.value)}
              ...
              placeholder={t("Creators of fitness in Guadalajara, 5k to 50k, TikTok")}
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <span style={{fontSize:11,color:C.muted,flexShrink:0}}>{t("or")}</span>
            <input value={linkInput} onChange={e=>{setLinkInput(e.target.value);setLinkErr(null);}}
              onKeyDown={e=>e.key==="Enter"&&addLink()}
              placeholder={t("Paste a TikTok or Instagram profile link")}
              style={{...inp,fontSize:12,padding:"8px 12px"}}/>
            <Btn onClick={addLink} disabled={!linkInput.trim()}>{t("Analyse")}</Btn>
          </div>
      ...
      {searching&&(
        <Card style={{marginBottom:16,textAlign:"center",padding:34}}>
          <div style={{fontSize:13,color:C.text,fontWeight:500,marginBottom:5}}>{t("Searching…")}</div>
          <div style={{fontSize:11,color:C.muted,lineHeight:1.6,maxWidth:380,margin:"0 auto"}}>{t("Only creators with a real, openable profile make it through.")}</div>
        </Card>
      )}

      {searchErr&&!searching&&(
        <Card style={{marginBottom:16}}>
          <div style={{fontSize:12,color:C.red,marginBottom:4,fontWeight:600}}>{t("Nothing usable came back")}</div>
      ...
      {results.length>0&&(
        <>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,gap:10,flexWrap:"wrap"}}>
            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>{t("Results")} · {results.length}</div>
            {selected.length>0&&(
              <button onClick={()=>setAddTo("choose")}
                ...
                {t("Add")} {selected.length} {t("to a list")}
              </button>
            )}
          </div>
              ...
              return(
                <div key={r.profile_url} onClick={()=>{if(!already)setSelected(p=>on?p.filter(x=>x!==i):[...p,i]);}}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                    ...
                    {already&&<span style={{fontSize:9,color:"#C2410C",background:"#FFF7ED",border:"0.5px solid #FED7AA",borderRadius:20,padding:"1px 7px",whiteSpace:"nowrap"}}>{t("Already saved")}</span>}
                  </div>
                  {r.why&&<div style={{fontSize:11,color:C.muted,lineHeight:1.5,marginBottom:8}}>{r.why}</div>}
                  {inLists.length>0&&<div style={{fontSize:10,color:C.muted,marginBottom:7}}>{t("In")} {inLists.map(l=>l.name).join(", ")}</div>}
                  <a href={r.profile_url} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
                    style={{fontSize:10,color:BRAND.blue,textDecoration:"none",border:`0.5px solid ${BRAND.blue}40`,borderRadius:20,padding:"3px 9px"}}>{t("View profile ↗")}</a>
                </div>
              );
            })}
          </div>
      ...
      <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:9}}>{t("Lists")}</div>
          ...
          return(
            <Card key={l.id} pad={15} style={{cursor:"pointer"}} onClick={()=>setOpenList(l)}>
              <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:10}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:10,color:C.muted,marginTop:2}}>
                    {l.client_id?`${clientEmoji(l.client_id)} ${clientName(l.client_id)} · `:""}{rows.length} {t("creators")}
                  </div>
                </div>
                <span style={{fontSize:10,color:C.muted,whiteSpace:"nowrap"}}>{t("Open")} →</span>
              </div>
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap",fontSize:10,color:C.muted}}>
                {counts.length?counts.map(c=><span key={c.id} style={{color:c.id==="contratado"?BRAND.green:C.muted}}>{c.n} {t(c.label)}</span>)
                  :<span>{t("Nobody here yet.")}</span>}
              </div>
            </Card>
          );
        })}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:16}}
          ...
          <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shMd,width:"min(420px,100%)",padding:22}}>
            <div style={{fontSize:15,fontWeight:600,color:C.text,marginBottom:4}}>
              {addTo==="new"?t("New list"):t("Add to a list")}
            </div>
            <div style={{fontSize:12,color:C.muted,lineHeight:1.55,marginBottom:16}}>
              {addTo==="new"?t("Name the campaign and pick the client it's for."):t("Pick an existing list, or create one.")}
            </div>
            ...
            <div style={{fontSize:11,color:C.muted,marginBottom:4,fontWeight:500}}>{t("List name")}</div>
            <input value={newList.name} onChange={e=>setNewList(p=>({...p,name:e.target.value}))}
              placeholder={t("Solanum summer")} style={{...inp,marginBottom:12}}/>
            <div style={{fontSize:11,color:C.muted,marginBottom:4,fontWeight:500}}>{t("Client")}</div>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              <Btn onClick={()=>setAddTo(null)}>{t("Cancel")}</Btn>
              <Btn primary disabled={!newList.name.trim()} onClick={createList}>{t("Create ✓")}</Btn>
            </div>
          </div>
        </div>
      )}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:16}}
      ...
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shMd,width:"min(420px,100%)",padding:22}}>
        ...
        <div style={{fontSize:12,color:C.muted,lineHeight:1.55,marginBottom:16}}>
          {t("Open the profile and type what you see. Dated, so everyone knows how fresh it is.")}
        </div>
        <a href={creator.profile_url} target="_blank" rel="noopener noreferrer"
          style={{display:"inline-block",fontSize:11,color:BRAND.blue,textDecoration:"none",marginBottom:14}}>{t("View profile ↗")}</a>
        {[["followers",t("Followers")],["avg_views",t("Average views")],["engagement",t("Engagement %")]].map(([k,label])=>(
        ...
        <div style={{fontSize:11,color:C.muted,marginBottom:4,fontWeight:500}}>{t("Notes")}</div>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          ...
          <Btn primary onClick={async()=>{
            await sbUpdate("creators","id",creator.id,{
