    ...
    <div>
      <Card pad={0} style={{marginBottom:16,overflow:"hidden"}}>
        ...
        <div style={{padding:"18px 20px 20px"}}>
          ...
          <div style={{fontSize:17,fontWeight:600,color:C.text,letterSpacing:-0.2,marginBottom:4}}>{t("What do you want to make?")}</div>
          ...
          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{t("Find references on")}</div>
          ...
          {builderOpen&&(
            <div style={{border:`0.5px solid ${C.border}`,borderRadius:10,background:C.light,padding:"12px 13px",marginBottom:12}}>
              <div style={{display:"flex",alignItems:"center",marginBottom:11}}>
                <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",flex:1}}>{t("Build your brief")}</div>
                <button onClick={()=>setBuilderOpen(false)}
                  style={{background:"none",border:"none",padding:0,cursor:"pointer",fontSize:11,color:C.muted}}>{t("Hide")}</button>
              </div>

              <div style={{fontSize:10,color:C.muted,marginBottom:4}}>{t("What are you promoting?")}</div>
              <input
                value={bWhat}
              ...
              <div style={{fontSize:10,color:C.muted,marginBottom:5}}>{t("What should it do?")}</div>
              ...
              <div style={{fontSize:10,color:C.muted,marginBottom:4}}>{t("Who's it for?")}</div>
              <input
                value={bAudience}
              ...
              <div style={{borderTop:`0.5px solid ${C.border}`,paddingTop:10}}>
                <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:5}}>{t("Your brief")}</div>
      ...
      {recent.length>0&&(
        <div>
          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>{t("Recent searches")}</div>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {recent.filter(c=>(c.title||"").trim()).map((c,i)=>(
              <div key={c.id} onClick={()=>{setInput(c.title);searchRefs(c.title);}}
                ...
                <span style={{fontSize:10,color:C.muted,flexShrink:0}}>{t("Search again \u2192")}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <div>
      ...
      {refs.length===0&&!streaming&&(
        <div style={{textAlign:"center",padding:"56px 24px",border:`1px dashed ${C.border}`,borderRadius:12,background:C.light}}>
          ...
          <button onClick={reset} style={{padding:"9px 20px",background:C.text,color:"#FFF",border:"none",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:700}}>{t("Try another search")}</button>
        </div>
      )}
    ...
    <div>
        ...
        <div style={{padding:"16px 20px 18px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,marginBottom:12}}>
            <div style={{minWidth:0}}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>{t("Ideas from your picks")}</div>
            ...
            <button onClick={reset} style={{padding:"6px 12px",border:`0.5px solid ${C.border}`,borderRadius:7,background:C.surface,cursor:"pointer",fontSize:12,color:C.muted,flexShrink:0}}>{t("New search")}</button>
          </div>
      ...
      {insights.count>0&&(
        <Card style={{marginBottom:16}}>
          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>{t("What your picks had in common")}</div>
      ...
      <Card style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,gap:10,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <span style={{fontSize:11,color:C.muted}}>{t("Pick the ones you want to make")}</span>
