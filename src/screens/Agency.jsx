  ...
  if(step===0)return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"min(500px,100%)",background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:shMd,padding:"40px 36px",textAlign:"center"}}>
        ...
        <div style={{fontSize:24,fontWeight:900,color:C.text,letterSpacing:-0.5,marginBottom:10,lineHeight:1.2}}>{t("Welcome to Sidekick Agency")}</div>
    ...
    <div>
      {clientError&&(
        <div style={{background:"#FEF2F2",border:`1px solid ${C.red}35`,borderRadius:10,padding:"11px 14px",marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:600,color:C.red,marginBottom:3}}>{t("That client didn't save")}</div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:14,marginBottom:10,flexWrap:"wrap"}}>
              ...
              {agencyGoal>0&&(
                <div style={{textAlign:"right"}}>
                  ...
                  <div style={{fontSize:10,color:C.muted,marginTop:2}}>{t("published")}</div>
                </div>
              )}
            </div>
            {agencyGoal>0&&(
      ...
      {clients.length>0&&(
        <>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:7,flexWrap:"wrap",gap:8}}>
            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>{t("By client \u00b7 tap to open a pipeline")}</div>
            <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>
              {AGENCY_STAGES.map(st=>(
                <span key={st.id} style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:9,color:C.muted}}>
                  <span style={{width:7,height:7,borderRadius:2,background:st.color}}/>{t(st.label==="Ready for Review"?"Review":st.label)}
                </span>
              ))}
            </div>
          </div>
      ...
      {clients.length>0&&(
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:7,gap:8}}>
          <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>{t("Schedule")}</div>
      ...
      {clients.length===0?(
        <Card style={{textAlign:"center",padding:48}}>
          ...
          <div style={{fontSize:15,fontWeight:700,color:C.text,marginBottom:6}}>{t("No clients yet")}</div>
          <div style={{fontSize:13,color:C.muted,marginBottom:20}}>{t("Add your first client to start building their content pipeline.")}</div>
          <Btn primary onClick={()=>setShowAddClient(true)}>{t("+ Add your first client")}</Btn>
        </Card>
      ):calOpen?(
        <>
          ...
          {unscheduled.length>0&&(
            <div style={{display:"flex",alignItems:"center",gap:11,background:C.surface,border:`1px solid ${C.border}`,borderLeft:`3px solid ${BRAND.yellow}`,borderRadius:9,padding:"11px 14px"}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:10,color:C.muted,marginTop:2}}>{t("They don't appear on the calendar until someone schedules them.")}</div>
              </div>
          ...
          {trayOpen&&unscheduled.length>0&&(
            <Card pad={12} style={{marginTop:8}}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>{t("Drag onto a day to schedule")}</div>
              ...
              {(byDay[dayOpen]||[]).map(v=>{
                ...
                return(
                  <div key={v.id} onClick={()=>{if(c)onSelectClient(c);}}
                    ...
                    <div style={{fontSize:10,color:C.muted}}>{c?c.name:""} · {t((AGENCY_STAGES.find(s=>s.id===v.stage)||{}).label||"")}</div>
                  </div>
                );
              })}
            </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui"}}>
          <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shMd,width:"min(380px,95vw)",padding:26}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:15,fontWeight:800,color:C.text}}>{t("Add a client")}</div>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><Btn onClick={()=>setShowAddClient(false)}>{t("Cancel")}</Btn><Btn onClick={addClient} primary disabled={adding||!newName.trim()}>{t("Add client \u2713")}</Btn></div>
          </div>
        </div>
      )}
    </div>
    ...
    <div>
      ...
      <Card style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:8}}>
          ...
          {goal>0&&<div style={{textAlign:"right"}}><div style={{fontSize:24,fontWeight:900,color:C.text}}>{pub}<span style={{fontSize:13,color:C.muted}}>/{goal}</span></div><div style={{fontSize:11,color:C.muted}}>{t("published")}</div></div>}
        </div>
          ...
          return(
            <div key={stage.id}
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}>
                <div style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:7,height:7,borderRadius:"50%",background:stage.color}}/><div style={{fontSize:10,fontWeight:700,color:C.text,lineHeight:1.2}}>{t(stage.label)}</div></div>
                  ...
                  <div key={v.id} draggable
                    ...
                    <div style={{padding:"10px 12px 0",flex:1,minWidth:0,position:"relative"}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                        ...
                        <span style={{fontSize:10,fontWeight:500,color:needsRevision?"#EA580C":isRev?"#F97316":stage.color,letterSpacing:0.2}}>{t(isRev?"Ready for Review":stage.label)}</span>
                      ...
                      {isPub&&!hasMet&&!unlocked&&(
                        <div style={{marginBottom:8,padding:"7px 9px",background:"#F8FAFC",borderRadius:7,border:`1px solid ${C.border}`}}>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:3}}>
                            ...
                            <span onClick={()=>setOverrideVid(v)} style={{fontSize:9,color:C.accent,cursor:"pointer",textDecoration:"underline"}}>{t("Already have data?")}</span>
                          </div>
                        </div>
                      )}
                      {isPub&&!hasMet&&unlocked&&(
                        <div style={{marginBottom:8,padding:"7px 9px",background:"#DCFCE7",borderRadius:7,border:"1px solid #BBF7D0",display:"flex",alignItems:"center",gap:6}}>
                          ...
                          <div><div style={{fontSize:11,fontWeight:700,color:C.green}}>{t("Ready to add metrics")}</div><div style={{fontSize:9,color:"#15803D"}}>7 days of data</div></div>
                        </div>
                      )}
                      ...
                      <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
                        ...
                        {stage.id==="approved"&&<button onClick={()=>handleMove(v.id,"published")} style={{fontSize:10,padding:"5px 11px",background:BRAND.green,border:"none",borderRadius:20,cursor:"pointer",color:"#FFF",fontWeight:600}}>{t("Publish \u2192")}</button>}
                        ...
                        {isCf&&<div style={{display:"flex",gap:4,alignItems:"center",marginLeft:"auto"}}><span style={{fontSize:10,color:C.red,fontWeight:700}}>{t("Delete?")}</span><button onClick={()=>{onDeleteVideo(v.id);setConfirmDel(null);}} style={{fontSize:10,padding:"2px 7px",background:C.red,border:"none",borderRadius:20,cursor:"pointer",color:"#fff",fontWeight:700}}>{t("Yes")}</button><button onClick={()=>setConfirmDel(null)} style={{fontSize:10,padding:"2px 7px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>{t("No")}</button></div>}
                      </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 20px",height:50,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,boxShadow:sh}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{background:"#EDE9FE",color:"#7C3AED",fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:20}}>{t("Agency")}</span>
