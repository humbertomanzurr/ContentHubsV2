        ...
        ${g.shots.map((s,i)=>`
          <div class="shot">
            ...
            ${s.kind==="line"
              ...
              :`<div class="line">${esc(s.text||"(describe the shot)")}</div><div class="who">{t("B-roll \u00b7 no dialogue")}</div>`}
          </div>`).join("")}
      </div>`).join("");
      ...
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 24px 80px rgba(0,0,0,.3)",width:"min(1120px,100%)",height:"min(740px,95vh)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{padding:"10px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:10,flexShrink:0,background:"#FAFAFA"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>{t("Shoot plan")}</div>
          ...
          <button onClick={printSheet} style={{fontSize:10,padding:"5px 11px",background:C.surface,border:`0.5px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>{t("Print call sheet")}</button>
        ...
        <div style={{flex:1,display:"grid",gridTemplateColumns:"185px 1fr 250px",overflow:"hidden"}}>
          ...
          <div style={{borderRight:`1px solid ${C.border}`,background:"#FAFAFA",overflowY:"auto",padding:"11px 11px 16px"}}>
            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{t("Talent")}</div>
            ...
            <input value={newTalent} onChange={e=>setNewTalent(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTalent()}
              placeholder={t("+ Add talent")} style={{...inp,fontSize:11,padding:"6px 8px",marginBottom:14,background:C.surface}}/>

            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{t("Locations")}</div>
            <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:8}}>
            ...
            <input value={newLocation} onChange={e=>setNewLocation(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addLocation()}
              placeholder={t("+ Add location")} style={{...inp,fontSize:11,padding:"6px 8px",marginBottom:14,background:C.surface}}/>
                  ...
                  <>
                    ...
                    <div style={{display:"flex",gap:4,marginBottom:6}}>
                      <input value={freeSearch} onChange={e=>setFreeSearch(e.target.value)}
                        ...
                        placeholder={t("Search your own words")} style={{...inp,fontSize:10,padding:"5px 8px",background:C.surface}}/>
                    </div>
          ...
          <div style={{overflowY:"auto",padding:"11px 14px 18px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9,gap:8}}>
              ...
              <button onClick={()=>insertBrollAt(plan.shots.length)} style={{fontSize:10,padding:"4px 10px",background:C.surface,border:`0.5px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>{t("+ B-roll at the end")}</button>
            </div>

            {plan.shots.length===0&&(
              <div style={{textAlign:"center",padding:"40px 20px",color:C.muted}}>
                <div style={{fontSize:13,color:C.text,fontWeight:500,marginBottom:5}}>{t("No shots yet")}</div>
          ...
          <div style={{borderLeft:`1px solid ${C.border}`,display:"flex",flexDirection:"column",overflow:"hidden",background:"#FAFAFA"}}>
            <div style={{flex:"0 0 58%",overflowY:"auto",padding:"11px 12px",borderBottom:`1px solid ${C.border}`}}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:3}}>{t("Shooting order")}</div>
              <div style={{fontSize:10,color:C.muted,lineHeight:1.5,marginBottom:9}}>{t("Grouped by location \u2014 set up once, get everything there.")}</div>
              ...
              {unassigned.length>0&&(
                <div style={{background:C.surface,border:`0.5px solid ${C.border}`,borderLeft:`3px solid ${BRAND.yellow}`,borderRadius:7,padding:"8px 10px"}}>
                  ...
                  <div style={{fontSize:10,color:C.muted,lineHeight:1.45}}>{t("Missing talent or a location.")}</div>
                </div>
              )}
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",padding:"10px 12px",gap:7,overflow:"hidden",minHeight:0}}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>{t("Ask anything")}</div>
              ...
              <div style={{display:"flex",gap:5,flexShrink:0}}>
                <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendChat()}
                  placeholder={t("Ask about coverage, order, pacing\u2026")}
              ...
              <div style={{fontSize:9,color:C.muted,textAlign:"center"}}>{t("Asks questions \u00b7 never plans it for you")}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:"8px 16px",borderTop:`0.5px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0,background:"#FAFAFA"}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={onClose} style={{padding:"6px 14px",background:C.surface,border:`0.5px solid ${C.border}`,borderRadius:7,cursor:"pointer",fontSize:12,color:C.text}}>{t("Close")}</button>
            <button onClick={()=>onSave(JSON.stringify(plan))} style={{padding:"6px 14px",background:"#111",color:"#FFF",border:"none",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:500}}>{t("Save \u2713")}</button>
          </div>
        </div>
      </div>
    </div>
