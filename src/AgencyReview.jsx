        ...
        <div style={{padding:"14px 18px",borderBottom:`0.5px solid ${C.border}`,flexShrink:0,background:"#FAFAFA"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
            <div style={{minWidth:0}}>
              <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:3}}>{t("Notes")}</div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px 16px",display:"flex",flexDirection:"column",gap:9}}>
          ...
          {!loading&&notes.length===0&&(
            <div style={{textAlign:"center",padding:"48px 20px",color:C.muted}}>
              ...
              <div style={{fontSize:13,color:C.text,fontWeight:500,marginBottom:4}}>{t("No notes yet")}</div>
              <div style={{fontSize:11,lineHeight:1.6,maxWidth:220,margin:"0 auto"}}>{t("Leave feedback for the editor, or approve the video if it's ready.")}</div>
            </div>
          )}
          {notes.map((n,i)=>{
              ...
              <div key={i} style={{display:"flex",gap:9}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                    ...
                    {isSendBack&&<span style={{fontSize:9,fontWeight:600,color:"#C2410C",background:"#FFF7ED",border:"0.5px solid #FED7AA",borderRadius:20,padding:"1px 7px"}}>{t("Sent back")}</span>}
                    {isFix&&<span style={{fontSize:9,fontWeight:600,color:BRAND.green,background:"#F0FBF6",border:`0.5px solid ${BRAND.green}40`,borderRadius:20,padding:"1px 7px"}}>{t("Fixed")}</span>}
                  </div>
        ...
        {video.stage==="review"&&(
          <div style={{padding:"11px 16px",borderTop:`0.5px solid ${C.border}`,flexShrink:0,background:"#FAFAFA"}}>
            {!showSendBack?(
              ...
              <div>
                <div style={{fontSize:10,fontWeight:600,color:C.muted,letterSpacing:.6,textTransform:"uppercase",marginBottom:6}}>{t("What needs to change?")}</div>
                <textarea value={sendBackNote} onChange={e=>setSendBackNote(e.target.value)}
                  placeholder={t("Be specific \u2014 the editor only sees this note.")}
                  style={{...inp,fontSize:12,height:66,resize:"none",lineHeight:1.55,marginBottom:8,background:C.surface}}/>
                <div style={{display:"flex",gap:7}}>
                  <button onClick={()=>{setShowSendBack(false);setSendBackNote("");}} style={{flex:1,padding:"8px 0",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,cursor:"pointer",fontSize:12,color:C.text}}>{t("Cancel")}</button>
                  <button onClick={handleSendBack} disabled={!sendBackNote.trim()}
                    style={{flex:1,padding:"8px 0",background:sendBackNote.trim()?"#F97316":C.border,color:sendBackNote.trim()?"#FFF":C.muted,border:"none",borderRadius:8,cursor:sendBackNote.trim()?"pointer":"not-allowed",fontSize:12,fontWeight:600}}>{t("Send back")}</button>
                </div>
              </div>
            )}
          </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.62)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui,sans-serif",padding:14}}
      ...
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 24px 80px rgba(0,0,0,.3)",width:"min(1040px,100%)",height:"min(700px,94vh)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{padding:"12px 18px",borderBottom:`0.5px solid ${C.border}`,display:"flex",alignItems:"flex-start",gap:12,flexShrink:0,background:"#FAFAFA"}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:3}}>{t("Review")}</div>
        ...
        <div style={{flex:1,display:"grid",gridTemplateColumns:"1fr 330px",overflow:"hidden"}}>
          ...
          <div style={{background:"#0B0B0C",display:"flex",flexDirection:"column",overflow:"hidden"}}>
            <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",minHeight:0,padding:10}}>
              ...
              {emb.kind==="none"&&(
                <div style={{textAlign:"center",color:"#9CA3AF",padding:24}}>
                  <div style={{fontSize:13,marginBottom:6}}>{t("No video attached yet")}</div>
                  <div style={{fontSize:11,lineHeight:1.6,maxWidth:280}}>{t("The editor adds the finished video when they send it for review.")}</div>
                </div>
              )}
            </div>
            <div style={{padding:"8px 12px",borderTop:"1px solid rgba(255,255,255,.08)",display:"flex",alignItems:"center",gap:9,flexShrink:0}}>
              {canAutoStamp?(
                <>
                  ...
                  <span style={{fontSize:10,color:"#6B7280"}}>{t("notes attach to this moment")}</span>
                </>
              ):emb.kind==="embed"?(
                <>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"13px 14px",display:"flex",flexDirection:"column",gap:8}}>
              ...
              {!loading&&stamped.length===0&&(
                <div style={{textAlign:"center",padding:"40px 16px",color:C.muted}}>
                  <div style={{fontSize:12,color:C.text,fontWeight:500,marginBottom:4}}>{t("No notes yet")}</div>
                  <div style={{fontSize:11,lineHeight:1.6}}>{t("Play the video and leave feedback at the moment it matters.")}</div>
                </div>
              )}
              {stamped.map((n,i)=>{
                  ...
                  <div key={i} style={{display:"flex",gap:8}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>
                        ...
                        <span style={{fontSize:11,fontWeight:600,color:C.text}}>{n.author_name||"Team"}</span>
                        <span style={{fontSize:10,color:C.muted}}>{timeAgo(n.created_at)}</span>
                        {isBack&&<span style={{fontSize:9,fontWeight:600,color:"#C2410C",background:"#FFF7ED",border:"0.5px solid #FED7AA",borderRadius:20,padding:"1px 6px"}}>{t("Sent back")}</span>}
                      </div>
              ...
              {!showSendBack?(
                <div style={{display:"flex",gap:7}}>
                  ...
                  <button onClick={()=>{onApprove(video.id);onClose();}}
                    style={{flex:1,padding:"9px 0",background:BRAND.green,border:"none",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600,color:"#FFF"}}>{t("Approve \u2713")}</button>
                </div>
              ):(
                <div>
                  <div style={{fontSize:10,fontWeight:600,color:C.muted,letterSpacing:.6,textTransform:"uppercase",marginBottom:6}}>{t("What needs to change?")}</div>
                  <textarea value={sendBackNote} onChange={e=>setSendBackNote(e.target.value)}
                    placeholder={t("The editor only sees this note.")}
      ...
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shMd,width:"min(440px,95vw)",padding:24}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:5}}>
          <div>
            ...
            <div style={{fontSize:15,fontWeight:600,color:C.text}}>{t("Attach the finished video")}</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer",lineHeight:1}}>×</button>
        </div>
        <div style={{fontSize:12,color:C.muted,marginBottom:14,lineHeight:1.55}}>{t("Paste a link the reviewer can open \u2014 Drive, YouTube, Vimeo, or a direct file.")}</div>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          ...
          <Btn primary disabled={!ok} onClick={()=>{onSave(url.trim());onClose();}}>{t("Send for review \u2192")}</Btn>
        </div>
      </div>
    </div>
  );
