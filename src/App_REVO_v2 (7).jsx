...
function KanbanCard({card,clients,role,onMove,onMetrics,onEarlyMetrics,onDelete,isDragging,onDragStart,onDragEnd,onMoveMonth,onAddVideoUrl}){
  ...
  return(
    <div draggable onDragStart={e=>{e.dataTransfer.setData("cardId",card.id);onDragStart&&onDragStart(card.id);}} onDragEnd={()=>onDragEnd&&onDragEnd()}
      ...
      <div style={{fontSize:11,color:C.muted,marginBottom:8}}>
        ...
        {card.referenceUrl&&<div><a href={card.referenceUrl} target="_blank" rel="noreferrer" style={{color:C.accent,textDecoration:"none",fontWeight:600}}>📎 Ver referencia Canva</a></div>}
        {card.videoUrl&&card.stage==="aprobacion"&&<div style={{marginTop:4}}><a href={card.videoUrl} target="_blank" rel="noreferrer" style={{color:C.green,textDecoration:"none",fontWeight:600,background:"#DCFCE7",padding:"3px 8px",borderRadius:20,display:"inline-block"}}>▶ Ver video</a></div>}
      </div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        ...
        {/* Move to month */}
        {role==="admin"&&<button onClick={()=>onMoveMonth&&onMoveMonth(card)} style={{fontSize:11,padding:"4px 10px",background:C.light,border:`1px solid ${C.border}`,borderRadius:6,cursor:"pointer",color:C.muted}}>📅 Mover mes</button>}
        {/* Add video URL in aprobacion */}
        {card.stage==="aprobacion"&&role==="admin"&&!card.videoUrl&&<button onClick={()=>onAddVideoUrl&&onAddVideoUrl(card)} style={{fontSize:11,padding:"4px 10px",background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:6,cursor:"pointer",color:C.accent,fontWeight:600}}>🔗 Agregar video</button>}
...
// ── PIPELINE BOARD ────────────────────────────────────────────────────────────
function PipelineBoard({clientId,clients,employees,cards,videos,targets,role,onAddCard,onMoveCard,onMetrics,onDeleteCard,onBack,activeMonth,onMonthChange,onSetTargets}){
  const month=activeMonth||curMonth();
          ...
          <div key={stage.id}
            ...
            {cardsInStage(stage.id).map(card=>(
              <KanbanCard key={card.id} card={card} clients={clients} role={role}
                ...
                onMoveMonth={c=>setMoveMonthCard(c)} onAddVideoUrl={c=>setAddVideoUrlCard(c)}/>
            ))}
